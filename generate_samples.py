...
Run this first to generate sample images before demo-ing the app.
"""
...
try:
    ...
    QR_AVAILABLE = False
    print("WARNING: qrcode library not installed. QR codes won't be generated.")

os.makedirs("samples", exist_ok=True)

# ── Answer key for Set-C ──────────────────────────────────────────────────────
QR_PAYLOAD = (
...
STUDENTS = [
    {"name": "Ali Hassan",    "reg": "FA22-BSE-041",
     "p1": ["D","A","B","A","D","A","A","B"],  # all correct
     "p2": ["C","D","D","D","C","C","C","B"]},
    {"name": "Sara Ahmed",   "reg": "FA22-BSE-042",
     "p1": ["D","A","C","A","D","B","A","B"],  # 2 wrong
     "p2": ["C","D","D","A","C","C","C","B"]}, # 1 wrong
    {"name": "Usman Khan",   "reg": "FA22-BSE-043",
     "p1": ["D","B","B","A","D","A","A","B"],  # 1 wrong
     "p2": ["C","D","B","D","C","A","C","B"]}, # 2 wrong
    {"name": "Fatima Malik", "reg": "FA22-BSE-044",
     "p1": ["A","A","B","A","D","A","A","B"],  # 1 wrong
     "p2": ["C","D","D","D","C","C","C","B"]}, # all correct
    {"name": "Zain Raza",    "reg": "FA22-BSE-045",
     "p1": ["D","A","B","C","A","A","A","B"],  # 2 wrong
     "p2": ["C","D","D","D","B","C","C","A"]}, # 2 wrong
]
...
def draw_quiz_sheet(student: dict, index: int) -> np.ndarray:
    ...
    cv2.putText(img, "SP2026   Time: 20 min   Marks: 16   Set-C", (30, 65), font, .5, GRAY, 1)
    ...
    if QR_AVAILABLE:
        ...
        cv2.putText(img, "QR CODE", (W-10-qr_size+20, 10+qr_size//2), font, .5, BLACK, 1)
    ...
    cv2.putText(img, "Class: BSE-4A    Subject: Artificial Intelligence", (30, y0+65), font, .45, GRAY, 1)

    # ── Divider ───────────────────────────────────────────────────────────────
    cv2.line(img, (30, 200), (W-30, 200), (200, 200, 200), 1)

    def draw_bubble_section(part_label: str, answers: list, x_off: int, answers_key: list):
        """Draw a block of 8 questions with A/B/C/D bubbles."""
        col_w = 50
        row_h = 52
        bub_r = 16
        y_start = 230

        cv2.putText(img, part_label, (x_off, y_start - 15), font, .65, BLACK, 2)

        # Option labels
        for j, opt in enumerate(OPTIONS):
            cv2.putText(img, opt, (x_off + 50 + j * col_w + bub_r - 5, y_start + 10),
                        font, .45, GRAY, 1)

        for i, (student_ans, _correct) in enumerate(zip(answers, answers_key)):
            y = y_start + 20 + i * row_h
            # Question number
            cv2.putText(img, f"Q{i+1:02d}", (x_off, y + bub_r), font, .45, BLACK, 1)
            for j, opt in enumerate(OPTIONS):
    ...
    # ── Footer ────────────────────────────────────────────────────────────────
    cv2.putText(img, "* Fill bubble completely | Use pencil | Do not fold",
